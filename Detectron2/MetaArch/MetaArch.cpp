	...
	m_input_format(cfg["INPUT.FORMAT"].as<string>())
{
	...
	if (name == "CenterNet")
	{
		m_centernet_generator =  shared_ptr<CenterNetImpl>(new CenterNetImpl(cfg, m_backbone->output_shapes()));
		register_module("centernet_generator", m_centernet_generator);
	}
	else
	{
