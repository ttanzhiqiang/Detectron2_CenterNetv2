...
torch::Tensor BaseMaskRCNNHeadImpl::mask_rcnn_loss(torch::Tensor pred_mask_logits, const InstancesList &instances,
	...
	for (auto &instances_per_image : instances) {
		...
		//SequencePtr Sequence_gt_masks = instances_per_image->get("gt_masks");
		//auto masks = std::dynamic_pointer_cast<Masks>(Sequence_gt_masks);

		auto gt_mask_tensor = instances_per_image->getTensor("gt_masks");
		int num = gt_mask_tensor.size(0);
		auto gt_mask_tensor_dx = gt_mask_tensor.index({ Colon, Slice(0, None, 2) });
		auto gt_mask_tensor_dy = gt_mask_tensor.index({ Colon, Slice(1, None, 2) });
		std::vector<TensorVec> m_gt_vec_tensort;
		for (int i = 0; i < gt_mask_tensor.size(0); i++)
		{
			TensorVec m_vec_tensor;
			auto p_tensor = stack({ gt_mask_tensor_dx[i] ,gt_mask_tensor_dy[i] }, 1);
			m_vec_tensor.push_back(p_tensor);
			m_gt_vec_tensort.push_back(m_vec_tensor);
		}
		PolygonMasks masks = PolygonMasks(m_gt_vec_tensort);
		auto gt_proposal_boxes = instances_per_image->getTensor("proposal_boxes");
		auto gt_masks_per_image = masks.crop_and_resize(gt_proposal_boxes, mask_side_len).to(pred_mask_logits.device());
		//cout << "gt_masks_per_image:" << gt_masks_per_image << endl;
	...
	auto false_positive = (mask_incorrect.bitwise_and(gt_masks_bool.bitwise_not())).sum().item<float>() / max(
