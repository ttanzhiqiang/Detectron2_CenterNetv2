...
void CascadeROIHeadsImpl::_init_box_head(CfgNode &cfg, const ShapeSpec::Map &input_shapes) {
	...
	auto box_heads = torch::nn::Sequential();
	auto box_predictors = torch::nn::Sequential();
...
std::tuple<TensorMap, InstancesList> CascadeROIHeadsImpl::_forward_box(const TensorMap &features_,
	...
	for (int k = 0; k < m_num_cascade_stages; k++) {
		...
		head_outputs.push_back({ m_box_predictors[k], std::move(predictions), proposals });
	}
	...
	if (is_training()) {
		...
		for (int i = 0; i < scores_per_stage[0].size(); i++) {
			...
			for (int j = 1; j < scores_per_stage.size(); j++) {
				scores_per_image = scores_per_stage[0][i];
...
TensorVec CascadeROIHeadsImpl::_run_stage(const TensorVec &features, InstancesList& proposals, int stage) {
	...
	auto box_features_1 = m_box_heads[stage](box_features);
	auto m_box_predictors_i = m_box_predictors[stage](box_features_1);
	return m_box_predictors_i;
}
