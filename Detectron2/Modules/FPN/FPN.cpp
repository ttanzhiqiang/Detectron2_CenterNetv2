...
#include <Detectron2/Modules/DLA/DLA.h>
#include <Detectron2/Modules/DLA/BiFPN.h>
...
Backbone Detectron2::build_backbone(CfgNode &cfg, const ShapeSpec *input_shape) {
	...
	if (backbone_name == "build_dla_backbone") {
		return build_dla_backbone(cfg, *input_shape);
	}
	if (backbone_name == "build_resnet_fpn_backbone") {
		return build_resnet_fpn_backbone(cfg, *input_shape);
	}
	if (backbone_name == "build_dla_fpn_backbone") {
		return build_dla_fpn_backbone(cfg, *input_shape);
	}
...
Backbone Detectron2::build_dla_fpn_backbone(CfgNode& cfg, const ShapeSpec& input_shape) {
	auto in_features = cfg["MODEL.FPN.IN_FEATURES"].as<vector<string>>();
	auto out_channels = cfg["MODEL.FPN.OUT_CHANNELS"].as<int64_t>();
	auto norm = BatchNorm::GetType(cfg["MODEL.FPN.NORM"].as<string>());
	auto num_repeats = cfg["MODEL.FPN.NUM_REPEATS"].as<int64_t>();
	auto fuse_type = cfg["MODEL.FPN.FUSE_TYPE"].as<string>();

	auto resnet = build_dla_backbone(cfg, input_shape);
	//std::cout << resnet << std::endl;
	...
	// creating FPN
	//return shared_ptr<BackboneImpl>(new FPNImpl(resnet, in_features, out_channels, norm, topBlock, fuse_type));
	return shared_ptr<BackboneImpl>(new BiFPNImpl(resnet, in_features, out_channels, num_repeats, norm));
}
	...
	m_fuse_type(fuse_type)
{
	...
	for (int i = 0; i < in_strides.size(); i++) {
		auto stage = m_stages[i];
		auto name = FormatString("dla%d", stage);  //"p%d"
