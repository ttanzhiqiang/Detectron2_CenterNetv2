...
void VisualizationDemo::start_val(const Options& options) {
	...
	auto num_classes = cfg["MODEL.CENTERNET.NUM_CLASSES"].as<int>();
	auto pr_scale = cfg["MODEL.ROI_HEADS.SCORE_THRESH_TEST"].as<float>();
	auto overlap_threshold = cfg["MODEL.ROI_HEADS.NMS_THRESH_TEST"].as<float>();
	...
	std::vector<int> tp(num_classes);
	std::vector<int> fp(num_classes);
	std::vector<int> fn(num_classes);
	std::vector<std::vector<float>> precisions;
	std::vector<std::vector<float>> recalls;
	precisions.resize(num_classes);
	recalls.resize(num_classes);
 	for (auto& img_datas : *dataloader) {
		...
		if (img_datas.size() == 1)
		{
			...
			for (int i = 0; i < num_classes; i++) {
				tp[i] += accumulator[i].TP();
				fp[i] += accumulator[i].FP();
				fn[i] += accumulator[i].FN();
				auto pr = Precision(tp[i], fp[i], fn[i]);
				auto recall = Recall(tp[i], fp[i], fn[i]);
				precisions[i].push_back(pr);
				recalls[i].push_back(recall);
			}
		}
	}
	float mean_avg_precision = 0;
	for (int i = 0; i < num_classes; i++) {
		std::cout << "tp: " << tp << " fp: " << fp << " fn: " << fn << std::endl;
		auto precision = Precision(tp[i], fp[i], fn[i]);
		auto recall = Recall(tp[i], fp[i], fn[i]);
		std::cout << "precision: " << precision << " recall: " << recall << std::endl;
		auto ap = compute_ap(precisions[i], recalls[i]);
		std::cout << "classes: " << i << " ap:" << ap << std::endl;
		mean_avg_precision += ap;
	}
	std::cout << "mean_avg_precision: " << mean_avg_precision << std::endl;
